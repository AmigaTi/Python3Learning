...
class Weather(object):
    def __init__(self, city=''):
        self.__url_base = 'https://api.heweather.com/x3'
    ...
    # https://api.heweather.com/x3/citylist?search=hotworld&key=59b0c90752ff4d67ace5bf52c9df90cc
    def get_citylist(self, search_type=SearchType.AllChina):
        ...
        # search_type.value = 0
        url = '%s/citylist?search=%s&key=%s' % (self.__url_base, self.__search[search_type.value], self.__key)
        logd('request: %s' % url)
        self.__citylist = self.handle_request(url)
        return self.__citylist

    # https://api.heweather.com/x3/weather?city=shenzhen&key=59b0c90752ff4d67ace5bf52c9df90cc
    def get_weather(self, city=''):
        if city is not None:
            self.__city = city
        url = '%s/weather?city=%s&key=%s' % (self.__url_base, self.__city, self.__key)
        logd('request: %s' % url)
